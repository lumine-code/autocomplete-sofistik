...
module.exports = {
   ...
   * file is for. Suggestions must be for the same release the linter judges
   * against and the tooling runs.
...
class Provider {
  ...
  /**
   * Set the environment service, which decides which release applies
   * @param {Object} service - The environment service
   */
  setEnvironmentService(service) {
    this.environmentService = service.provider;
    // The release may differ from what was assumed before it arrived.
    this.loadSuggestions();
  }
  ...
  /**
   * Keywords for the release this file is for.
   *
   * Which release that is belongs to `sofistik.environment` — asking there
   * rather than deciding here is what stops the suggestions offering words the
   * linter would reject or the tooling would not run.
   * @param {TextEditor} editor - Optional editor the completion is for
   * @returns {Object|null} Keywords bound to that release
   */
  keywordsFor(editor) {
    if (!this.keywordsService) {
      return null;
    }
    const resolved = this.environmentService?.resolve({ editor });
    return this.keywordsService.forRelease(resolved?.version, resolved?.language);
  }
  ...
  loadSuggestions() {
    ...
    try {
      ...
      const ctx = this.keywordsFor(null);
  ...
  findCommand(editor, bufferPosition) {
    ...
    try {
      ...
      const ctx = this.keywordsFor(editor);
